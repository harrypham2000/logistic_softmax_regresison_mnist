...
class LogisticClassifier(object):
    ...
    def compute_loss(self, y, y_hat):
        ...
        :param y_hat: the probabilities that the given samples belong to class 1
...
def normalize_per_pixel(train_x, test_x):
    ...
    # Calculate mean and standard deviation for each pixel across all training images
    ...
    # Normalize test set per pixel using the mean and standard deviation calculated from train set
...
def test(y_hat, test_y):
    ...
    #precision = TP/(TP+FP)
    TP=((y_hat>=0.5)&(test_y==1)).sum()
    FP=((y_hat>=0.5)&(test_y==0)).sum()
    TN=((y_hat<0.5)& (test_y== 1)).sum()
    FN=((y_hat<0.5)&(test_y==0)).sum()
    precision =TP/(FP+TP)
    #recall = TP/P
    recall = TP/(FN+TP)
...
if __name__ == "__main__":
    ...
    for e in range(num_epoch):
        ...
        if (e % epochs_to_draw == epochs_to_draw-1):
            plot_loss(all_loss)
            plt.savefig(f'/content/drive/MyDrive/HW2_ML/HW2_ML/plot_np_logistic_{e}.png')
            plt.show()
            plt.pause(0.1)
            plt.close()
